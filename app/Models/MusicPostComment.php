...
class MusicPostComment extends Model
{
    use HasFactory;
    use SerializeDate;
    ...
    // 参照させたいmusic_post_commentsを指定
    protected $table = 'music_post_comments';
    ...
    {
        return $this->belongsTo(MusicPost::class, 'music_post_id', 'id');
    }
    ...
    {
        return $this->belongsTo(MusicPostComment::class, 'parent_id', 'id');
    }

    // 子コメントとのリレーション 一対多の関係
    public function replies()
    {
        return $this->hasMany(MusicPostComment::class, 'parent_id', 'id');
    }

    // いいねをしたUserに対するリレーション　多対多の関係
    public function users()
    {
        return $this->belongsToMany(User::class, 'user_mp_comment', 'mp_comment_id', 'user_id')
