...
use App\Http\Requests\WpCommentRequest;
...
class WpCommentController extends Controller
{
    ...
    // 新しく記述した内容を保存する
    public function store(WorkPostComment $wp_comment, WpCommentRequest $request)
    {
        $input_comment = $request['work_post_comment'];
        ...
        $wp_comment->fill($input_comment)->save();

        // statusの確認
        $status = is_null($wp_comment->parent_id) ? 'comment_stored' : 'child_comment_stored';
        ...
        $commentHtml = view('user_interactions.comments.input_comment', [
            ...
            'postCommentId' => $wp_comment->work_post_id,
            'parentId' => $wp_comment->parent_id
        ])->render();
        $message = __('messages.comment_posted');
        return response()->json(['message' => $message, 'new_comment_id' => $wp_comment->id, 'commentHtml' => $commentHtml]);
    }

    // コメントを削除する
    public function delete(WorkPostComment $wp_comment, $comment_id)
    {
        try {
            $parentCommentArray = $wp_comment->getParentCommentArray($comment_id);
            // 子コメントがあれば全て削除
            ...
            return response()->json(['message' => $message, 'commentCount' => $cpmment_count], 200);
        } catch (\Exception $e) {
    ...
    {
        // コメントが見つからない場合の処理
        $comment = WorkPostComment::find($comment_id);
        if (!$comment) {
        ...
        // いいねしたユーザー数の取得
        $count = count($comment->users()->pluck('wp_comment_id')->toArray());

        return response()->json(['status' => $status, 'like_user' => $count, 'message' => $message]);
    }

    // ネスト化したコメントの表示
    public function replies(WorkPostComment $wp_comment, $comment_id)
    {
        $wp_comment = WorkPostComment::find($comment_id);
        $replies = $wp_comment->replies()->with('user', 'users', 'replies')->get();
        $replies = $replies->map(function ($reply) {
            ...
            $reply->html = view('user_interactions.comments.input_comment', [
                ...
                'baseRoute' => 'work_post',
                'inputPostIdName' => 'work_post_id',
                'postCommentId' => $reply->work_post_id,
    ...
    {
        // 編集の対象となるデータを取得
        $targetComment = WorkPostComment::find($comment_id);
        // 削除する投稿の画像も削除する処理
