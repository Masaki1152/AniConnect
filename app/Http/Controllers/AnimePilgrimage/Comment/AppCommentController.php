...
use App\Http\Requests\AppCommentRequest;
...
class AppCommentController extends Controller
{
    ...
    // 新しく記述した内容を保存する
    public function store(AnimePilgrimagePostComment $app_comment, AppCommentRequest $request)
    {
        $input_comment = $request['pilgrimage_post_comment'];
        ...
        $app_comment->fill($input_comment)->save();

        // statusの確認
        $status = is_null($app_comment->parent_id) ? 'comment_stored' : 'child_comment_stored';
    ...
    // コメントを削除する
    public function delete(AnimePilgrimagePostComment $app_comment, $comment_id)
    {
        try {
            $parentCommentArray = $app_comment->getParentCommentArray($comment_id);
            // 子コメントがあれば全て削除
    ...
    // ネスト化したコメントの表示
    public function replies(AnimePilgrimagePostComment $app_comment, $comment_id)
    {
        $app_comment = AnimePilgrimagePostComment::find($comment_id);
        $replies = $app_comment->replies()->with('user', 'users', 'replies')->get();
        $replies = $replies->map(function ($reply) {
            ...
            $reply->html = view('user_interactions.comments.input_comment', [
                ...
                'baseRoute' => 'pilgrimage_post',
                'inputPostIdName' => 'anime_pilgrimage_post_id',
                'postCommentId' => $reply->anime_pilgrimage_post_id,
    ...
    {
        // 編集の対象となるデータを取得
        $targetComment = AnimePilgrimagePostComment::find($comment_id);
        // 削除する投稿の画像も削除する処理
