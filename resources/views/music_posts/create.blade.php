        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl lg:max-w-2xl">
                    <h2 class="text-lg font-medium text-gray-900">「{{ $music->name }}」への新規感想投稿</h2>
                    <p class="mt-1 text-sm text-gray-600">楽曲の感想を書いてみんなと共有しましょう！</p>
                    ...
                    <form action="{{ route('music_posts.store', ['music_id' => $music->id]) }}" method="POST"
                        ...
                        <div class="star_num">
                            ...
                            <select name="music_post[star_num]"
                                ...
                                @foreach ($numbers as $num => $star)
                                    <option value="{{ $num }}"
                                        @if (old('music_post.star_num') == $num) selected @endif>
                                        {{ $star }}
                                    </option>
                                @endforeach
                            </select>
                            @if ($errors->has('music_post.star_num'))
                                <p class="mt-2 text-sm text-red-600">{{ $errors->first('music_post.star_num') }}</p>
                            @endif
                        </div>
                        <div id="custom-multi-select-container" class="category relative">
                            ...
                            <div id="selected-categories-container">
                                @foreach ($selectedCategories as $selectedCategory)
                                    <input type="hidden" name="music_post[categories_array][]"
                        ...
                        <div class="body">
                            ...
                            <textarea name="music_post[body]" placeholder="内容を記入してください。"
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 h-40"
                                data-max-length="4000" data-counter-id="bodyCharacterCount" oninput="countCharacter(this)">{{ old('music_post.body') }}</textarea>
                            <p id="bodyCharacterCount" class="mt-1 text-sm text-gray-500"></p>
                            <p class="text-sm text-red-500 mt-1">{{ $errors->first('music_post.body') }}</p>
                        </div>
                        <div class="image">
