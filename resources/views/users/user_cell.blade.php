<div class='user_cell w-full sm:w-full lg:w-[50%] flex items-start p-1 gap-2 bg-white shadow rounded-lg h-[170px]'>
    <!-- 左側のブロック -->
    <div class="left_block w-1/4 h-full pl-2 flex justify-center items-center">
        <!-- ユーザー画像 -->
        <div class='user_image w-34 h-34 rounded-full overflow-hidden flex items-center justify-center'>
            @if ($user->image)
                <div>
    ...
    <div class="follow w-3/4 h-full px-4 py-2 flex flex-col justify-between">
        ...
        <div class="top_block flex items-center justify-between">
            <h2 class='name text-lg font-semibold'>
                ...
                @else
                    <a href="{{ route('users.show', ['user_id' => $user->id]) }}" class="text-blue-500 hover:underline">
                        {{ $user->name }}
                    </a>
                @endif
            </h2>
