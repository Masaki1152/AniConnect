<x-app-layout>
    ...
    <div class="flex-cols">
        <h2>人気の音楽</h2>
        <ul>
            @foreach ($topPopularityMusic as $topPopularityMusicObject)
                <li>
                    <h3>{{ $topPopularityMusicObject['item']->name }}</h3>
                </li>
            @endforeach
        </ul>
    </div>
        ...
        @else
            ...
            @foreach ($music as $music_object)
                <div class='music'>
                    ...
                    <x-molecules.evaluation.star-num-detail :starNum="$music_object->average_star_num" :postNum="$music_object->post_num" />
                    <x-molecules.button.interested type="music" :root="$music_object" path="music.interested.index"
                        :prop="['music_id' => $music_object->id]" isMultiple="false" />
                </div>
            @endforeach
        @endif
    </div>
