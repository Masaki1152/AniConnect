                    ...
                    <form action="{{ route('character_posts.store', ['character_id' => $character->id]) }}" method="POST"
                        ...
                        <div class="star_num">
                            ...
                            <select name="character_post[star_num]"
                                ...
                                @foreach ($numbers as $num => $star)
                                    <option value="{{ $num }}"
                                        @if (old('character_post.star_num') == $num) selected @endif>
                                        {{ $star }}
                                    </option>
                                @endforeach
                            </select>
                            @if ($errors->has('character_post.star_num'))
                                <p class="mt-2 text-sm text-red-600">{{ $errors->first('character_post.star_num') }}</p>
                            @endif
                        </div>
                        ...
                        <div class="body">
                            ...
                            <textarea name="character_post[body]" placeholder="内容を記入してください。"
                                ...
                                data-max-length="4000" data-counter-id="bodyCharacterCount" oninput="countCharacter(this)">{{ old('character_post.body') }}</textarea>
                            <p id="bodyCharacterCount" class="mt-1 text-sm text-gray-500"></p>
                            <p class="text-sm text-red-500 mt-1">{{ $errors->first('character_post.body') }}</p>
                        </div>
                        <div class="image">
