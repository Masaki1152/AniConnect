        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl lg:max-w-2xl">
                    <h2 class="text-lg font-medium text-gray-900">「{{ $work_story_post->workStory->sub_title }}」への投稿編集画面
                    </h2>
                    <p class="mt-1 text-sm text-gray-600">あらすじの感想を編集してみんなと共有しましょう！</p>
                    <form
                        ...
                        @method('PUT')
                        <div class="work_id">
                            <input type="hidden" name="work_story_post[work_id]"
                                value="{{ $work_story_post->work_id }}">
                        </div>
                        <div class="user_id">
                            <input type="hidden" name="work_story_post[user_id]"
                                value="{{ $work_story_post->user_id }}">
                        </div>
                        ...
                        <div class="title">
                            ...
                            <input type="text" name="work_story_post[post_title]" placeholder="タイトル"
                                value="{{ old('work_story_post.post_title', $work_story_post->post_title) }}"
                            ...
                            <p class="title__error text-sm text-red-500 mt-1">
                                {{ $errors->first('work_story_post.post_title') }}</p>
                        </div>
                        <div class="star_num">
                            <label class="block font-medium text-sm text-gray-700 mb-2">星の数</label>
                            <select name="work_story_post[star_num]"
                                ...
                                @foreach ($numbers as $num => $star)
                                    <option value="{{ $num }}"
                                        @if ($work_story_post->star_num == $num) selected @endif>
                                        {{ $star }}
                                    </option>
                                @endforeach
                            </select>
                            @if ($errors->has('work_story_post.star_num'))
                                <p class="mt-2 text-sm text-red-600">{{ $errors->first('work_story_post.star_num') }}
                                </p>
                            @endif
                        </div>
                        <div id="custom-multi-select-container" class="category relative">
                            ...
                            <div id="selected-categories-container">
                                @foreach ($selectedCategories as $selectedCategory)
                                    <input type="hidden" name="work_story_post[categories_array][]"
                        ...
                        <div class="body">
                            ...
                            <textarea name="work_story_post[body]" placeholder="内容を記入してください。"
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 h-40"
                                data-max-length="4000" data-counter-id="bodyCharacterCount" oninput="countCharacter(this)">{{ old('work_story_post.body', $work_story_post->body) }}</textarea>
                            <p id="bodyCharacterCount" class="mt-1 text-sm text-gray-500"></p>
                            <p class="body__error text-sm text-red-500 mt-1">
                                {{ $errors->first('work_story_post.body') }}
                            </p>
                        </div>
                        <div class="image">
