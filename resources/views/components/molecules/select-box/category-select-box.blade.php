@php
    ...
    $selectedCategories = old("{$targetTableName}.categories_array", $existingCategories);
@endphp
<div id="custom-multi-select-container" class="category relative">
        ...
        <div id="custom-multi-select-list" class="max-h-48 overflow-y-auto">
            @foreach ($categories as $category)
                <div class="custom-option p-2 cursor-pointer @if (in_array($category->id, $selectedCategories)) bg-gray-500 text-white @endif"
                    data-value="{{ $category->id }}" data-post-type="{{ $targetTableName }}">
                    {{ $category->name }}
                </div>
            @endforeach
        </div>
    ...
    <div id="selected-categories-container">
        @foreach ($selectedCategories as $selectedCategory)
            <input type="hidden" name="{{ $targetTableName }}[categories_array][]" value="{{ $selectedCategory }}">
        @endforeach
    </div>
    @if ($errors->has("{$targetTableName}.categories_array"))
        <p class="category__error text-sm text-red-500 mt-1">
            {{ $errors->first("{$targetTableName}.categories_array") }}</p>
    @endif
</div>
